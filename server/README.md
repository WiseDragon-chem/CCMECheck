...
到点的那一轮之外，任务还会检查「本该有的最新截止日」有没有对应的可用快照：进程在
排行榜时间不在线（开发机夜里关机、线上崩溃重启）时那一轮会永久缺失，而排行榜只读
最新一份快照。落后的差额由**下一个整点或进程启动时**补上（启动补跑复用同一个任务，
所以也有 `job_runs` 记录）。到点之后刻意不做每小时重算：§9.2 要求 06:00 时仍待审核的
记录进入**下一次**快照，每小时重算会让当天白天通过的记录混进当天的榜单。
...
| 11 | §9.2「每天 06:00 生成快照」没说进程在该时刻不在线时怎么办 | 下一个整点／进程启动时补生成漏掉的那一轮；cutoff 公式不变，仍是「统计截至前一日」 |
